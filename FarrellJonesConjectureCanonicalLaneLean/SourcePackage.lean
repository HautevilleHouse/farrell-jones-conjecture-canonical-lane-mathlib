/-!
# Source package model for `farrell-jones-conjecture-canonical-lane`
...
def sourceCheckoutHead : String := "2e852b50c3ad2ed7a139f414418e634c8fc564db"
...
def sourceFiles : List SourceFile := [
  ...
  { path := "scripts/fj_closure_guard.py", sha256 := "b958c7118cc041dd4e482349987a8e4375b324ba44222e4dffe4379eb71b0975", functionCount := 9, classCount := 0, assignmentCount := 8, parseOk := true },
...
def sourceFunctions : List SourceFunctionDecl := [
  ...
  { file := "scripts/fj_closure_guard.py", name := "_finite", args := ["v"], returns := "bool", doc := "", line := 31, isAsync := false },
  { file := "scripts/fj_closure_guard.py", name := "_resolve", args := ["path_str"], returns := "Path", doc := "", line := 35, isAsync := false },
  { file := "scripts/fj_closure_guard.py", name := "_bootstrap_registry", args := ["path"], returns := "None", doc := "", line := 42, isAsync := false },
  { file := "scripts/fj_closure_guard.py", name := "_load_registry", args := ["path"], returns := "dict[str, Any]", doc := "", line := 48, isAsync := false },
  { file := "scripts/fj_closure_guard.py", name := "_load_sigma_from_stitch", args := ["path"], returns := "float | None", doc := "", line := 60, isAsync := false },
  { file := "scripts/fj_closure_guard.py", name := "_entry", args := ["constants", "key"], returns := "tuple[float | None, bool]", doc := "", line := 77, isAsync := false },
  { file := "scripts/fj_closure_guard.py", name := "compute_report", args := ["data", "sigma_star", "strict_coh_zero"], returns := "dict[str, Any]", doc := "", line := 88, isAsync := false },
  { file := "scripts/fj_closure_guard.py", name := "append_history", args := ["path", "report"], returns := "None", doc := "", line := 121, isAsync := false },
  { file := "scripts/fj_closure_guard.py", name := "main", args := [], returns := "None", doc := "", line := 128, isAsync := false },
...
def sourceAssignments : List SourceAssignmentDecl := [
  ...
  { file := "scripts/fj_closure_guard.py", name := "SCRIPT_DIR", value := "Path(__file__).resolve().parent", line := 13 },
  { file := "scripts/fj_closure_guard.py", name := "PROJECT_ROOT", value := "SCRIPT_DIR.parent", line := 14 },
  { file := "scripts/fj_closure_guard.py", name := "DEFAULT_REGISTRY", value := "\"artifacts/constants_registry.json\"", line := 16 },
  { file := "scripts/fj_closure_guard.py", name := "DEFAULT_STITCH", value := "\"artifacts/stitch_constants.json\"", line := 17 },
  { file := "scripts/fj_closure_guard.py", name := "DEFAULT_OUT", value := "\"repro/certificate_runtime.json\"", line := 18 },
  { file := "scripts/fj_closure_guard.py", name := "DEFAULT_HISTORY", value := "\"repro/drift_guard_runs.jsonl\"", line := 19 },
  { file := "scripts/fj_closure_guard.py", name := "PRIMARY_KEYS", value := "[\"kappa_assembly\", \"sigma_control\", \"kappa_compact\", \"rho_rigidity\", \"isomorphism_transfer\"]", line := 21 },
  { file := "scripts/fj_closure_guard.py", name := "COHERENCE_KEY", value := "\"eps_coh\"", line := 28 },
...
def constantSpecs : List ConstantSpec := [
  { group := "constants", key := "kappa_assembly", status := "derived_numeric", formula := "c_assembly_raw * assembly_density_raw - e_assembly_raw", sourceSection := "paper/FARRELL_JONES_CONJECTURE_PREPRINT.md Section 4.5 / Appendix A", validation := "required_positive", componentKeys := ["assembly_density_raw", "c_assembly_raw", "e_assembly_raw"] },
  { group := "constants", key := "sigma_control", status := "derived_numeric", formula := "control_floor_raw - coarse_loss_raw - restart_loss_raw", sourceSection := "paper/FARRELL_JONES_CONJECTURE_PREPRINT.md Section 5.2 / Appendix B", validation := "required_positive", componentKeys := ["coarse_loss_raw", "control_floor_raw", "restart_loss_raw"] },
  { group := "constants", key := "kappa_compact", status := "derived_numeric", formula := "1.0 / (1.0 + delta_comp_sup_raw)", sourceSection := "paper/FARRELL_JONES_CONJECTURE_PREPRINT.md Section 5.3 / Appendix C", validation := "required_positive", componentKeys := ["delta_comp_sup_raw"] },
  { group := "constants", key := "rho_rigidity", status := "derived_numeric", formula := "rho_rigidity_raw", sourceSection := "paper/FARRELL_JONES_CONJECTURE_PREPRINT.md Section 6.1 / Appendix D", validation := "required_positive", componentKeys := ["rho_rigidity_raw"] },
  { group := "constants", key := "isomorphism_transfer", status := "derived_numeric", formula := "c_iso_raw * transfer_gain_raw - e_iso_raw", sourceSection := "paper/FARRELL_JONES_CONJECTURE_PREPRINT.md Section 6.2 / Appendix E.4", validation := "required_positive", componentKeys := ["c_iso_raw", "e_iso_raw", "transfer_gain_raw"] },
  { group := "constants", key := "eps_coh", status := "derived_numeric", formula := "eps_coh_raw", sourceSection := "paper/FARRELL_JONES_CONJECTURE_PREPRINT.md Appendix E.6", validation := "required_nonnegative", componentKeys := ["eps_coh_raw"] },
  { group := "stitch", key := "sigma_star_can", status := "derived_numeric", formula := "sigma_star_can_raw", sourceSection := "paper/FARRELL_JONES_CONJECTURE_PREPRINT.md Section 5 / stitch constants", validation := "required_positive", componentKeys := ["sigma_star_can_raw"] }
]

def registryConstants : List RegistryConstant := [
  { key := "eps_coh", value := "0.0", theoremLevel := true, status := "derived_numeric", source := "artifacts/constants_extracted.json#eps_coh", sourceSection := "paper/FARRELL_JONES_CONJECTURE_PREPRINT.md Appendix E.6" },
  { key := "isomorphism_transfer", value := "1.029422", theoremLevel := true, status := "derived_numeric", source := "artifacts/constants_extracted.json#isomorphism_transfer", sourceSection := "paper/FARRELL_JONES_CONJECTURE_PREPRINT.md Section 6.2 / Appendix E.4" },
  { key := "kappa_assembly", value := "1.0918990000000002", theoremLevel := true, status := "derived_numeric", source := "artifacts/constants_extracted.json#kappa_assembly", sourceSection := "paper/FARRELL_JONES_CONJECTURE_PREPRINT.md Section 4.5 / Appendix A" },
  { key := "kappa_compact", value := "0.8045052292839904", theoremLevel := true, status := "derived_numeric", source := "artifacts/constants_extracted.json#kappa_compact", sourceSection := "paper/FARRELL_JONES_CONJECTURE_PREPRINT.md Section 5.3 / Appendix C" },
  { key := "rho_rigidity", value := "1.077", theoremLevel := true, status := "derived_numeric", source := "artifacts/constants_extracted.json#rho_rigidity", sourceSection := "paper/FARRELL_JONES_CONJECTURE_PREPRINT.md Section 6.1 / Appendix D" },
  { key := "sigma_control", value := "1.073", theoremLevel := true, status := "derived_numeric", source := "artifacts/constants_extracted.json#sigma_control", sourceSection := "paper/FARRELL_JONES_CONJECTURE_PREPRINT.md Section 5.2 / Appendix B" }
]
